...
vector<vector<string > > Eliminator::replace(int i){
	...
	for(int j = 0 ; j < (parser->non_terminal_defs->at(i)).size(); j++){
		if((parser->non_terminal_defs->at(i)).at(j).at(0).compare(parser->non_terminals->at(i))!=0){
			int index = find_index((parser->non_terminal_defs->at(i)).at(j).at(0));
			if(index < i){
				for(int k = 0; k <(parser->non_terminal_defs->at(index)).size() ;k++){
					vector<string> vec2;
//					vec.push_back(parser->non_terminal_defs.at(index).at(k));
					for(int z = 0; z < (parser->non_terminal_defs->at(index)).at(k).size(); z++){
						vec2.push_back((parser->non_terminal_defs->at(index)).at(k).at(z));
					}

					for(int z = 0 ; z < (parser->non_terminal_defs->at(i)).at(j).size(); z++){
						vec2.push_back((parser->non_terminal_defs->at(i)).at(j).at(z));
					}
...
void Eliminator::eliminate_from_updated(vector<vector<string> > replaced, int index){
	...
	for (int i = 0; i < (parser->non_terminal_defs->at(index)).size(); i++) {
		if(non_terminal_name.compare((parser->non_terminal_defs->at(index)).at(i).at(1))==0){
	...
	if(lr_indecies.size() == 0){
		...
		for(int i = 0; i < n_lr_indecies.size(); i++){
			vector<string> vec = (parser->non_terminal_defs->at(index)).at(n_lr_indecies.at(i));
		...
		for(int i = 0; i < lr_indecies.size(); i++){
			vector<string> vec;
			for(int k = 1; k < (parser->non_terminal_defs->at(index)).at(lr_indecies.at(i)).size();k++){
				vec.push_back((parser->non_terminal_defs->at(index)).at(lr_indecies.at(i)).at(k));
			}
			vec.push_back(new_non_terminal);
...
void Eliminator::eliminate_LR() {
	...
	for(int i = 0; i<parser->non_terminals->size(); i++){
		...
		for(int j = 0; j < parser->non_terminal_defs->at(i).size(); j++){
			for(int l =0; l<parser->non_terminal_defs->at(i).at(j).size(); l++){
